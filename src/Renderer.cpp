...
Renderer::Renderer(std::shared_ptr<VulkanInstance> vulkanInstance, std::shared_ptr<LogicalDevice> logicalDevice)
{
...
void Renderer::drawFrame(std::vector<VkSemaphore> &imageAvailableSemaphores, std::vector<VkSemaphore> &renderFinishedSemaphores, size_t &currentFrame)
{
	auto vkInstance = vkInst.lock();
	auto logDev = logicalDevice.lock();
	vkWaitForFences(logDev->device, 1, &vkInstance->deviceManager->inFlightFences[currentFrame], VK_TRUE, UINT64_MAX);
	
	uint32_t imageIndex;
	vkAcquireNextImageKHR(logDev->device, vkInstance->deviceManager->swapChain, UINT64_MAX,
						  imageAvailableSemaphores[currentFrame], VK_NULL_HANDLE, &imageIndex);

	if(vkInstance->deviceManager->imagesInFlight[imageIndex] != VK_NULL_HANDLE)
	{
		vkWaitForFences(logDev->device, 1, &vkInstance->deviceManager->imagesInFlight[imageIndex], VK_TRUE, UINT64_MAX);
	}
	...
	vkResetFences(logDev->device, 1, &vkInstance->deviceManager->inFlightFences[currentFrame]);

	if(vkQueueSubmit(logDev->graphicsQueue, 1, &submitInfo, vkInstance->deviceManager->inFlightFences[currentFrame]) != VK_SUCCESS)
	{
